...
const TEAM_TASKS: { id: string; owner: string; icon: string; tasks: { id: string; label: string }[] }[] = [
  { id: "nahuel",   owner: "Nahuel",   icon: "📞", tasks: [] },
  { id: "tadeo",    owner: "Tadeo",    icon: "✂️", tasks: [
    { id: "dt1", label: "Terminar edición videos de Velocity" },
    { id: "dt2", label: "Terminar edición videos NQN Importados" },
    { id: "dt4", label: "Editar y lanzar Jireh" },
  ]},
  { id: "luciano",  owner: "Luciano",  icon: "🤝", tasks: [] },
  { id: "santiago", owner: "Santiago", icon: "🎬", tasks: [
    { id: "dt3", label: "Grabar con Cuesta Azul" },
  ]},
];

const ALL_TASKS = TEAM_TASKS.flatMap((m) => m.tasks.map((t) => ({ ...t, owner: m.owner, icon: m.icon })));
...
export default function DashboardClient({ clients }: Props) {
  const { accounts, ready } = useAllAccounts(clients.map((c) => c.id));
  const { done, toggle, weekOffset, setWeekOffset, monday, isCurrentWeek } = useWeeklyTasks("velocity-weekly-tasks-v2");

  const today = new Date().toLocaleDateString("es-AR", {
    weekday: "long", year: "numeric", month: "long", day: "numeric",
  });

  const pendingTasks   = ALL_TASKS.filter((t) => !done[t.id]);
  const completedTasks = ALL_TASKS.filter((t) =>  done[t.id]);
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm mt-1" style={{ color: "#8b90a7" }}>Velocity Performance · Panel general</p>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="rounded-xl overflow-hidden" style={{ border: "1px solid #2e3349" }}>
            ...
            {pendingTasks.length === 0 ? (
              <div className="px-5 py-6 text-sm text-center" style={{ color: "#8b90a740" }}>
              ...
              <div className="flex flex-col">
                {pendingTasks.map((task, i) => (
                  <div
                    ...
                    style={{ borderTop: i > 0 ? "1px solid #2e334940" : "none" }}
                  >
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => toggle(task.id)}
                        className="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all"
                        style={{ borderColor: "#2e3349", background: "transparent", cursor: "pointer" }}
                      />
                      <span className="text-sm" style={{ color: "#e8eaf0" }}>{task.label}</span>
                    </div>
                    <span className="text-xs flex-shrink-0 ml-4" style={{ color: "#8b90a750" }}>{task.icon} {task.owner}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="flex flex-col">
                {completedTasks.map((task, i) => (
                  <div
                    ...
                    style={{ borderTop: i > 0 ? "1px solid #2e334930" : "none" }}
                  >
                    ...
                    <button
                      ...
                      style={{ color: "#8b90a7", background: "transparent", border: "1px solid #2e3349", cursor: "pointer" }}
                    >deshacer</button>
                  </div>
                ))}
              </div>
